const rawIds = [
  ...
  "89a08160-a01d-461f-89b3-4634f4fd32c7", // <-- 30th ID (index 29)
  "9041e4f7-f805-4ec1-be2b-6419178823af", // <-- 31st ID (index 30)
...
]; // Note: rawIds is now a simple array.
...
// __ MAIN QUESTIONS ARRAY ___
const questions = rawIds.map((id, index) => {
  let subject;
  // type को 'mcq' मान लिया है क्योंकि सभी answers (A, B, C, D) options में हैं
  const type = "mcq"; 
  
  // Subject logic (0-29 Maths, 30-59 SST)
  if(index < 30) {
    subject = "Maths";
  } else if(index < 60) {
    subject = "SST";
  ...
  // Ensure we have a correct answer for this index
  ...
  return {
    ...
    correct: correct,
    subject: subject || "Unknown", // Fallback for safety
    type
  };
});

console.log(questions); // Check the final output

// ----- (Optional: for verification) -------
questions.forEach((q, i) => {
  if (q.correct !== correctAnswers[i]) console.error(`Mismatch Q${q.id}`);
});
